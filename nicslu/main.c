...
//	ret = NicsLU_ReadTripletRowToSparse("m7.mtx", &n, &nnz, &ax, &ai, &ap);
...
//
//	for (int i = 0; i < nicslu->n; i++) {
//		printf("%1.3f ",nicslu->ldiag[i]);
//	}
//	printf("\n");
//	unsigned char* arr = (unsigned char* )nicslu->lu_array;
//	for (int i = 0; i < nicslu->n; i++) {
//		int* llen = nicslu->llen;
//		int* ulen = nicslu->ulen;
//		printf("%d: ",i);
...
int main(int argc, char *argv[])
{
	...
	ret = NicsLU_ReadTripletRowToSparse("diag.mtx", &n, &nnz, &ax, &ai, &ap);
	//ret = NicsLU_ReadTripletColumnToSparse("ASIC_100k.mtx", &n, &nnz, &ax, &ai, &ap);
	if (ret != NICS_OK) goto EXIT;
